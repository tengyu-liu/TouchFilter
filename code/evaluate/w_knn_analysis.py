...
tsne = TSNE()
w_manifold = tsne.fit_transform(data['syn_w'])

plt.scatter(w_manifold[:,0], w_manifold[:,1], s=1, c='b')
plt.show()


knn = KDTree(data['syn_w'])

i = random.randint(0, len(data['syn_w'])-1)
w = data['syn_w'][i]

neighbor_distance, neighbor_indices = knn.query([w], 5, return_distance=True, sort_results=True)
...
ax.scatter(w_manifold[:,0], w_manifold[:,1], s=1, c='blue')
ax.scatter(w_manifold[neighbor_indices,0], w_manifold[neighbor_indices,1], s=5, c='red')
ax.axis('off')
