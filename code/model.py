...
class Model:
    ...
    def build_model(self):
        ...
        self.obs_ewpp = {i: self.descriptor(self.obs_z, self.cup_r, self.cup_models[i], reuse=(i!=1)) for i in self.cup_list}
        self.langevin_dynamics = {i : self.langevin_dynamics_fn(i) for i in self.cup_list}
        self.inp_ewpp = {i: self.descriptor(self.inp_z, self.cup_r, self.cup_models[i], reuse=True) for i in self.cup_list}
        self.syn_zewpp = {i: self.langevin_dynamics[i](self.inp_z, self.cup_r) for i in self.cup_list}

        self.descriptor_loss = {i : self.obs_ewpp[i][0] - self.inp_ewpp[i][0] + (self.obs_ewpp[i][3] + self.inp_ewpp[i][3]) * self.penalty_strength for i in self.cup_list}
        pass
