...
class Visualizer:
    ...
    def visualize_distance(self, cup_id, hand_z, save_path):
        cup_model = tm.load_mesh(os.path.join(os.path.dirname(__file__), '../../data/cups/onepiece/%d.obj'%cup_id))
        z_ = hand_z
        jrot = z_[:22]
    ...
    def visualize_weight(self, obj_id, hand_z, obj_rot, obj_trans, hand_w, save_path=None, ax=None, c='white'):
        cup_model = tm.load_mesh(os.path.join(os.path.dirname(__file__), '../../data/cups/onepiece/%d.obj'%obj_id))
        obj_rot4 = np.eye(4)
        obj_rot4[:3,:3] = obj_rot
        cup_model.apply_transform(obj_rot4)
        cup_model.apply_translation(obj_trans)
        z_ = hand_z
        jrot = z_[:22]
        grot = np.reshape(z_[22:28], [3, 2])
        gpos = z_[28:]
