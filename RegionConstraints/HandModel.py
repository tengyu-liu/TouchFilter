...
class HandModel:
  def __init__(
    ...
    mano_path='../ForceClosure/third_party/manopth/mano/models', n_contact=3):
    ...
    if os.path.exists('../ForceClosure/mano_manifold_distances.pkl'):
      self.mano_manifold_distances = pickle.load(open('../ForceClosure/mano_manifold_distances.pkl', 'rb'))
    else:
      self.mano_manifold_distances = self.compute_manifold_distances()
      pickle.dump(self.mano_manifold_distances, open('../ForceClosure/mano_manifold_distances.pkl', 'wb'))
  ...
  def get_texture_coords(self):
    old_mano_verts = trimesh.load('../ForceClosure/mano.obj', process=False).vertices
    texture_verts, texture_coords = self.load_obj('../ForceClosure/mano_with_uv_full.obj')
  ...
  def test_texture(self):
    ...
    texture = imread('../ForceClosure/mano.png') / 255.
    w = texture.shape[0]
    h = texture.shape[1]
    v = trimesh.load('../ForceClosure/mano.obj', process=False).vertices
