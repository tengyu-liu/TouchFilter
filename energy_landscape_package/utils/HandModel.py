...
class HandModel:
    def __init__(self, batch_size):
        ...
        self.surface_pts = {p: tf.constant(np.load(os.path.join(os.path.dirname(__file__), '../data/hand', p + '.sample_points.npy')), dtype=tf.float32) for p in self.parts if '0' not in p}
        self.pts_normals = {p: tf.constant(np.load(os.path.join(os.path.dirname(__file__), '../data/hand', p + '.sample_normal.npy')), dtype=tf.float32) for p in self.parts if '0' not in p}
        self.pts_feature = tf.tile(tf.expand_dims(tf.concat([tf.constant(np.load(os.path.join(os.path.dirname(__file__), '../data/hand', p + '.sample_feat.npy')), dtype=tf.float32) for p in self.parts if '0' not in p], axis=0), axis=0), [batch_size, 1, 1])
