...
class VisUtil:
    def __init__(self, size=[640, 480], offscreen=True):
        ...
        stl_dict = {obj: np.load(os.path.join(obj_base, 'hand/%s.sample_points.npy'%obj)) for obj in self.parts}
        ...
        self.cup_models = {i: tm.load_mesh(os.path.join(obj_base, 'cup/%d.obj'%i)) for i in [3]}
    ...
    def visualize(self, hand_z):
        mlab.clf()
        cup_model = self.cup_models[3]
        mlab.triangular_mesh(cup_model.vertices[:,0], cup_model.vertices[:,1], cup_model.vertices[:,2], cup_model.faces, color=(0, 1, 0), opacity=0.5)
