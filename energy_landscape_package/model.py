...
class Model:
    ...
    def build_config(self, config, mean, stddev, cup_list):
        ...
        self.neg_jrot_penalty = tf.constant(100, dtype=tf.float32)

        self.debug = config.debug

        self.cup_list = cup_list

        self.cup_model_path = os.path.join(os.path.dirname(__file__), 'data/cup')
    ...
    def descriptor(self, hand_z, z2, cup_model, penetration_penalty):
        ...
        elif self.prior_type == 'Natural':
            hand_prior = self.hand_prior_natural(hand_z)
        else:
