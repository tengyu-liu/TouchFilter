...
class GraspPrediction(nn.Module):
  ...
  # * 3 friction vectors
  # * hand code
  def __init__(self, num_cp, hand_code_length):
    super(GraspPrediction, self).__init__()
    self.h1 = nn.Linear(256 + 10, 1024)
    self.h2 = TwoLinear(1024)
    self.h3 = TwoLinear(1024)
    self.contact_point_pred = nn.Linear(1024, num_cp * 3, bias=False)
    self.contact_region_pred = LinearGenerator(num_cp, 8)
